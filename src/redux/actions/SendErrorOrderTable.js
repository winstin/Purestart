/**
 @author Mothpro
**/
export const SENDERROR_GET_ORDER_DATA = "SENDERROR_GET_ORDER_DATA"
export const SENDERROR_ITEMS_CHECK_ARR = "SENDERROR_ITEMS_CHECK_ARR"
export const SENDERROR_ISLOADING = "SENDERROR_ISLOADING"
import {api} from "./AY_API"

export function checkedOnChange(ItemsSelected_arr){
    return (
        {
            type:"SENDERROR_ITEMS_CHECK_ARR",
...
export function getOrderData(condition){
    ...
    return (dispatch,getState)=>{
            let isLoading = getState().SendErrorOrderTable.isLoading;
            //加载动画
            isLoading?'':dispatch({'type':'SENDERROR_ISLOADING'});
            api("ebs.orderData.list",condition,function(e){
                console.log("SendErrorOrderTable",e);
                //遍历数据源，修改数据结构并返回勾选集合
